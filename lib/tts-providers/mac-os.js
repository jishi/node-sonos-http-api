...
function macSay(phrase, voice) {
  if (!settings.macDefaultVoice) {
    return Promise.resolve();
  }

  var selectedVoice = settings.macDefaultVoice;
  if( voice != undefined ) {
    selectedVoice = voice;
  }
  ...
  const filename = `macSay-${phraseHash}-${selectedVoice}.m4a`;
 ...
 return new Promise((resolve, reject) => {
    ...
    //
    exec(`say -v ${selectedVoice} "${phrase}" -o ${filepath}`,
