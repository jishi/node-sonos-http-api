...
function loadTracks(type, tracksJson)
{
  ...
  if (tracksJson.length > 0) {
    if (type == 'album') {
      tracks.queueTracks.push({albumName:tracksJson[0].albumName, uri:tracksJson[0].uri, metadata:tracksJson[0].metadata});
      tracks.count = 1;
    } else {
  ...
  let getChunk = (chunk) => {
    chunk.items.reduce(function(tracksArray, item) 
    {
      if (loadingTracks) {
        ...
        result.items.push({artistTrackSearch:item.artist+' '+item.title, trackName:item.title, artistName:item.artist, albumName:item.album, uri:item.uri, metadata:metadata});
      } else {
        var metadataID = libraryDef.metastart['album'] + encodeURIComponent(item.title);
        var metadata = getMetadata('album', metadataID, '');           
        result.items.push({albumName:item.title, uri:item.uri, metadata:metadata});
      }
    }, []);

    result.numberReturned += chunk.numberReturned;
    result.totalMatches = chunk.totalMatches;
    console.log("Count " + result.numberReturned);
...
function loadLibrarySearch(player, load) {
  if (load || (musicLibrary == null)) {
    return loadLibrary(player)
      ...
      .then(() => loadFuse(musicLibrary.albums.items, ["albumName"]))
      .then((result) => {
        fuzzyAlbums = result;
...
function handleLibrary(err, data) {
  if (!err) {
    ...
    if (musicLibrary.tracks == undefined) { // Ignore if old format
      musicLibrary = null;
    }
