...
const SPOTIFY_ACCESS_TOKEN_KEY = 'SPOTIFY_ACCESS_TOKEN_KEY';
...
function getHeaders() {
  const accessToken = store.get(SPOTIFY_ACCESS_TOKEN_KEY);
  if (accessToken == null) {
    return null;
  }
  return {
...
function authenticateService() {
  ...
  return new Promise((resolve, reject) => {
    spotifyAuth(SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET).then((response) => {
      const { accessToken } = response;
      store.set(SPOTIFY_ACCESS_TOKEN_KEY, accessToken);
      resolve();
    }).catch(reject);
  });
}
