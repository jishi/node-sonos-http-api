...
function getURI(type, id) {
  ...
  if (type == 'song') {
    return `x-sonos-http:tr-flac%3a${id}.flac?sid=${sid}&flags=8224&sn=${accountSN}`;
  } else
  if (type == 'station') {
...
function getMetadata(type, id, name, title) {
  ...
  const parentUri = eliteDef.parent[type] + name;
  const objectType = eliteDef.object[type];
...
function getURIandMetadata(type, resList)
{
  ...
  MetadataID = eliteDef.metastart[type] + encodeURIComponent(Id);
...
function loadTracks(type, tracksJson)
{
  ...
  if (tracksJson.data.length > 0) {
    ...
    tracks.queueTracks = tracksJson.data.reduce(function(tracksArray, track) {
      ...
      if (!skip) {
        var metadataID = eliteDef.metastart['song'] + encodeURIComponent(track.id);
