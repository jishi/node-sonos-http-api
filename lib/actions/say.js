...
// announce volume
var announceVolume = settings.say.announceVolume || 40;

function say(player, values, callback) {

  var text = values[0],
    language = values[1] || 'en-gb',
    volume = values[2] || announceVolume;
...
function sayAll(player, values, callback) {
  ...
  allPlayers.push({ roomName: biggestZone.coordinator.roomName, volume: volume });
  ...
  biggestZone.members.forEach(function (i) {
    ...
    allPlayers.push({ roomName: i.roomName, volume: volume });
  });
