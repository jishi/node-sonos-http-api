...
function spotify(player, values) {
  ...
  console.log(action, uri, metadata);
  ...
  if (action == 'queue') {
    ...
    player.coordinator.addURIToQueue(uri, metadata, true, nextTrackNo, function (success, res) {
      console.log(res)
      if (success) player.coordinator.nextTrack();
    });
  } else if (action == 'next') {
    var nextTrackNo = player.coordinator.state.trackNo + 1;
