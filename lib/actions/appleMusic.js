...
// Information for performing iTunes HTTP search.  Limit can be anything from 1 to 200, which will affect how many tracks are returned
var optionsiTunes = {
  basePath: "/search?media=music&limit=50&entity=song&term=",
...
function appleMusic(player, values) {
  ...
  // Parse the iTunes search results and load queue with tracks.  The search name can be an artist name OR a song title  
  var handleiTunesResponse = function (error, tracksJson) {
    if (!error) {
      try { 
        tracksJson = JSON.parse(tracksJson);
        console.log("Found " + tracksJson.resultCount + " tracks for " + trackID);
        if (tracksJson.resultCount > 0) {
          ...
          if (trkNum < tracksJson.resultCount) {
            ...
            if (trackCount > artistCount) {
              ...
              }, 2000);  // Wait 2 seconds after removing tracks
            } else {
              addTrackToQueue(tracksJson.results[trkNum], true);
            }
          }
  ...
  if (action == 'queue') {
    if (type == 'name') {
      optionsiTunes.path = optionsiTunes.basePath + trackID.split(':')[1];
      httpreq(optionsiTunes, handleiTunesResponse);
    } else {
      player.coordinator.addURIToQueue(uri, metadata);
    }
  } else if(action == 'now') {
