...
function appleMusic(player, values) {
  ...
  if ((action == 'radio') || ((action == 'queue') && (type == 'name'))) {
    if (action == 'radio') {
      return request({
        url: 'https://sticky-summer-lb.inkstone-clients.net/api/v1/searchMusic?limit=1&media=appleMusic&entity=station&term=' + trackID.split(':')[1],
        json: true
      })
        .then((radioStations) => {
      ...
      return request({
        url: 'https://itunes.apple.com/search?media=music&limit=50&entity=song&term=' + trackID.split(':')[1],
        json: true
      })
        .then(function (tracksJson) {
          if (tracksJson.resultCount == 0) {
            ...
            var trkNum = tracksJson.results.filter(track => !track.isStreamable).length;

            // If no streamable tracks
            if (trkNum == tracksJson.resultCount) {
              return Promise.reject('No tracks were streamable');
            } else {
              var artistCount = 1;
              var trackCount = 1;
              var artists = tracksJson.results.map((track) => track.artistName.toLowerCase()).sort();
              var tracks = tracksJson.results.map((track) => track.trackName.toLowerCase()).sort();
              ...
              if (trackCount / artistCount > 2) {
                ...
                return player.coordinator.clearQueue()
                  ...
                  .then(() => {
                    ...
                    queueTracks.slice(1).reduce((promise, track, index) => {
