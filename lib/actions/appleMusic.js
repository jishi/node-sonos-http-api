...
var metadataTemplate = '<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/" ' +
        ...
        '<item id="{metadataID}" parentID="{parentURI}" restricted="true">{title}<upnp:class>object.item.audioItem.{clazz}</upnp:class>' +
...
function appleMusic(player, values) {
  ...
  if (action == 'radio') {
    request({url: "https://sticky-summer-lb.inkstone-clients.net/api/v1/searchMusic?limit=1&media=appleMusic&entity=station&term=" + trackID.split(':')[1],
            json: true})
      .then(function(radioStations) {
        if (radioStations.resultCount > 0) {
          var radioID = radioStations.results[0].id;
          var radioTitle = radioStations.results[0].name;
          var radioName = radioTitle.toLowerCase().replace(' radio','');
          var metadataID = METADATA_URI_STARTERS[type] + encodeURIComponent('radio:' + radioID);     
          var metadata = metadataTemplate.format({
                metadataID: metadataID,
                parentURI: PARENTS[type] + radioName,
                clazz: CLASSES[type],
                title: "<dc:title>" + radioStations.results[0].name + "</dc:title>"
              });
          var uri = uriTemplates[type].format({id: 'radio%3a' + radioID});
          
          player.setAVTransportURI(uri, metadata, function (error) {
            if (!error) {
            ...
            if (trackCount/artistCount > 2) {
                ...
                async.eachSeries(tracksJson.results,
                  function(track,callback) {
                    ...
                    if (track.isStreamable) {
                      ...
                      for (var j=0; (j < trkNum-1) && !found ; j++) {
              ...
              player.coordinator.addURIToQueue(uri, metadata, true, nextTrackNo, function (error, res) {
                if (!error) {
                  ...
                  var queueURI = 'x-rincon-queue:' + player.uuid + '#0';
                  player.setAVTransportURI(queueURI, '', function () {
                    player.coordinator.seek(nextTrackNo,function (error) {
                      player.coordinator.play();
                    });
                  });
                }
