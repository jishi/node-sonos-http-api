...
function isolate(player, values) {
  ...
  // If playerToIsolate isn't playing, add it to the playing group
  if(playerToIsolate.state.currentState != "PLAYING") {
...
function performIsolate(discovery, playerToIsolate) {
  // Isolate playerToIsolate by calling becomeCoordinatorOfStandaloneGroup() on every other player in the currently playing group
