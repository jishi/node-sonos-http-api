...
const musicServices = ['apple','spotify','deezer','deezere','library'];
...
function doSearch(service, type, term)
{
  ...
  if ((type == 'song') || (service == 'library') || (service.startsWith('deezer') && (type == 'album'))) {
    ...
    if (term.indexOf(':') > -1) {
      ...
      if ((service == 'spotify') || (service.startsWith('deezer'))) {
        if ((service.startsWith('deezer')) && (album != '')) {
          newTerm = album + ' ';
        }
        if (artist != '') {
      ...
      newTerm = encodeURIComponent(term);
    }
...
function getAccountId(player, service)
{
  ...
  if (service != 'library') {
    return request({url: player.baseUrl + '/status/accounts',json: false})
      .then((res) => {
        var actLoc = res.indexOf(serviceTypes[service]);
...
const serviceTokens = {
  ...
  deezere:'SA_RINCON43015_',
  library: 'RINCON_AssociatedZPUDN'
};

function getMetadata(service, type, id, name, title) {
  const token = serviceTokens[service] + ((service.startsWith('deezer'))?accountId:'');
...
function getSongUri(service, id) {
  if (service == 'apple') {
    return `x-sonos-http:song%3a${id}.mp4?sid=204&flags=8224&sn=${accountSN}`;
  } else
  if (service == 'spotify') {
	return `x-sonos-spotify:spotify%3atrack%3a${id}?sid=12&flags=8224&sn=${accountSN}`;
  } else
  if (service == 'deezer') {
	return `x-sonos-http:tr%3a${id}.mp3?sid=168&flags=8224&sn=${accountSN}`;
  } else
  if (service == 'deezere') {
	return `x-sonos-http:tr-flac%3a${id}.flac?sid=168&flags=8224&sn=${accountSN}`;
  }
}

function getStationUri(service, id) {
  if (service == 'apple') {
    return `x-sonosapi-radio:radio%3a${id}?sid=204&flags=8300&sn=${accountSN}`;
  } else
  if (service == 'spotify') {
	return `x-sonosapi-radio:spotify%3aartistRadio%3a${id}?sid=12&flags=8300&sn=${accountSN}`;
  } else
  if (service == 'deezer') {
	return `x-sonosapi-radio:radio-artist-${id}?sid=168&flags=104&sn=${accountSN}`;
  } else
  if (service == 'deezere') {
	return `x-sonosapi-radio:radio-artist-${id}?sid=168&flags=104&sn=${accountSN}`;
  }
  ...
  if (service.startsWith('deezer')) {
    if (tracksJson.data.length > 0) {
      ...
      tracks.queueTracks = tracksJson.data.reduce(function(tracksArray, track) {
        ...
        if (!skip) {
          var metadataID = METADATA_URI_STARTERS[service]['song'] + encodeURIComponent(track.id);
          var metadata = getMetadata(service, 'song', metadataID, track.title.toLowerCase(), track.title);
          var uri = uriTemplates['song'](service, encodeURIComponent(track.id));
...
function musicSearch(player, values) {
  ...
  return getAccountId(player, service)
    .then(() => doSearch(service, type, term))
    .then((resList) => {
