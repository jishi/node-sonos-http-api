...
function doSearch(service, type, term)
{
  ...
  if ((type == 'song') || (service == 'library') || (((service == 'deezer') || (service == 'elite')) && (type == 'album'))) {
    ...
    if (term.indexOf(':') > -1) {
      ...
      if ((service == 'spotify') || (service == 'deezer') || (service == 'elite')) {
        if (((service == 'deezer') || (service == 'elite')) && (album != '')) {
          newTerm = album + ' ';
        }
        if (artist != '') {
  ...
  if (service == 'library') {
    newTerm = decodeURIComponent(newTerm);  
...
const serviceTokens = {
  ...
  elite:   'SA_RINCON43015_',
  library: 'RINCON_AssociatedZPUDN'
};

function getMetadata(service, type, id, name, title) {
  const token = serviceTokens[service] + (((service == 'deezer') || (service == 'elite'))?accountId:'');
...
function getURIandMetadata(player, service, type, resList)
{
  ...
  if ((service == 'deezer') || (service == 'elite')) {
