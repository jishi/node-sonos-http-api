...
function getTuneInMetadata(uri, serviceType) {
  return `<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/"
        ...
        <item id="F00092020${uri}" parentID="L" restricted="true"><dc:title>tunein</dc:title><upnp:class>object.item.audioItem.audioBroadcast</upnp:class>
...
function tuneIn(player, values) {
  ...
  const uri = `x-sonosapi-stream:${encodedTuneInUri}?sid=${sid}&flags=8224&sn=0`;

  if (!tuneInUri.startsWith('s')) {
