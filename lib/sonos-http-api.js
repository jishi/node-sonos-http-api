...
function HttpAPI(discovery, settings, presets) {
  ...
  function createMetadata(uri) {
    // Find correct metadata.

    if (uri.indexOf('x-sonos-spotify') == 0) {
      // this is Spotify, return correct metadata
        return '';
    }
  ...
  function handleAction(options, callback) {
    ...
    switch (options.action.toLowerCase()) {
      ...
      case "favorites":
        ...
        return;
      case "enqueuespotify":
        var uri = 'x-sonos-spotify:' + encodeURIComponent(options.value) + '?sid=9&flags=32&sn=1';
        ...
        '<item id="00030020' + encodeURIComponent(options.value) + '" restricted="true"><upnp:class>object.item.audioItem.musicTrack</upnp:class>' +
        '<desc id="cdudn" nameSpace="urn:schemas-rinconnetworks-com:metadata-1-0/">SA_RINCON2311_X_#Svc2311-0-Token</desc></item></DIDL-Lite>';
        player.coordinator.addURIToQueue(uri, metadata, function (success) {
          console.log(success);
        });
      ...
      case "playnowspotify":
        ...
        '<item id="00030020' + encodeURIComponent(options.value) + '" restricted="true"><upnp:class>object.item.audioItem.musicTrack</upnp:class>' +
        '<desc id="cdudn" nameSpace="urn:schemas-rinconnetworks-com:metadata-1-0/">SA_RINCON2311_X_#Svc2311-0-Token</desc></item></DIDL-Lite>';
        player.coordinator.setAVTransportURI(uri, metadata, function (success) {
